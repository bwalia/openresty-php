...
if [ -z ${VERSION} ];
then
  echo "openresty119 php74 docker image version not set"
  exit 1
fi
...
DOCKER_IMAGE_NAME=${DOCKER_REG_HUB_USER}/openresty119php74
DOCKER_PUBLIC_IMAGE_NAME=bwalia/openresty119php74
...
docker build -t ${DOCKER_PUBLIC_IMAGE_NAME}:${VERSION} -f devops/docker/Dockerfile .
